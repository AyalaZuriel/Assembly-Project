...
#define MAX_NUM_IMMEDIATE (4096) /*max possible integer on 12 bits*/
#define MIN_NUM_IMMEDIATE (-4096) /*min possible integer on 12 bits*/
#define REGISTERS_NUM 8 /*number of registers*/
#define BASE_NUM 10 /*base of a regular number*/
...
void directiveManager(int directive, int line_counter, int *newDC, bool ifHasLabelFlag, int *newIC){
    ...
    if (directive == DATA_DIRECTIVE){
        ...
        if (deleteSpaces(&currentChar, FIRST_DELETE)==EMPTY_SPACE){
            ...
            if((flagRemove=deleteSpaces(&currentChar, NORMAL_DELETE)) != REMOVED_SPACES){
                ...
                /* loop through the different data words */
                while (flag != FALSE && currentChar){
                    /* check that the token is a valid number */
    ...
    /* string directive */
        ...
        if(deleteSpaces(&currentChar, FIRST_DELETE) !=REMOVED_SPACES){
            ...
            int i=0;
            /* check that the string starts with double quotes */
            if (currentChar[i]!='\"'){
                ...
                /* check that the string ends with double quotes */
                if (currentChar[i]!='\"'){
                    addToErrorList("A string in the string directive needs to end with double quoets", line_counter, ERROR);
                }
                else{
                    i++;
                    /* check that there is no extra content after the string */
    ...
    /* extern directive */
        ...
        if ((flagRemove= deleteSpaces(&currentChar, NORMAL_DELETE)) != REMOVED_SPACES){
            ...
            /* insert the external label symbol into the symbol table */
